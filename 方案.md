...
export class PaddleAPIEngine implements OCREngine {
    ...
    async recognize(base64Data: string): Promise<string> {
        ...
        const response = await requestUrl({
            ...
            body: JSON.stringify({
                file: base64Data,
                fileType: 1,                              // 1 = 图片（PDF 场景为 0，后续扩展用）
                useDocOrientationClassify: false,
                useDocUnwarping: false,
                useChartRecognition: false,
            }),
...
export class OCRView extends ItemView {
    ...
    async onOpen() {
        ...
        // ── 加载态遮罩（覆盖在图片上方）──
        // 用 setText / createEl 而非 innerHTML：Obsidian 社区插件审核禁止 innerHTML（XSS 风险）
        this.loadingOverlay = this.dropZone.createEl('div', { cls: 'ocr-loading-overlay' });
        this.loadingOverlay.setText('🔍 识别中...');
        this.loadingOverlay.style.display = 'none';

        this.placeholder = this.dropZone.createEl('div', { cls: 'ocr-placeholder' });
        this.placeholder.createSpan({ text: '📷 拖拽图片到此处' });
        this.placeholder.createEl('br');
        this.placeholder.createSpan({ text: '或 ' });
        this.placeholder.createEl('kbd', { text: 'Ctrl+V' });
        this.placeholder.createSpan({ text: ' 粘贴' });
        ...
        // 用 this.registerDomEvent() 而非裸 addEventListener：视图关闭时 Obsidian 自动解绑，避免泄漏
        ...
        // ── 快捷键：Ctrl/Cmd+Enter 复制（兼容 macOS）──
